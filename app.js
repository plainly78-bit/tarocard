...
function dealNextCard() {
  ...
  cardImg.src = `cards/${randomCard.img}`;
  ...
  filtered.forEach(card => {
    ...
    cardEl.innerHTML = `
      <div class="card-border-gold">
        ...
        <div class="card-img-container">
          <img src="cards/${card.img}" alt="${card.nameKo}" class="card-artwork-img" loading="lazy">
        </div>
        <div class="card-footer-mini">
          <h3>${card.nameKo}</h3>
...
function openStudyModal(cardId) {
  ...
  elements.modalCardImg.src = `cards/${card.img}`;
  ...
  history.forEach(item => {
      ...
      if (cardObj) {
        thumbsHTML += `
          <div class="mini-thumb" title="${cardObj.nameKo}">
            <img src="cards/${cardObj.img}" alt="${cardObj.nameKo}">
          </div>
        `;
      }
    });
    ...
    itemEl.addEventListener('click', () => {
      ...
      item.cards.forEach((saved, idx) => {
        ...
        slotEl.querySelector('.card-artwork-img').src = `cards/${cardObj.img}`;
